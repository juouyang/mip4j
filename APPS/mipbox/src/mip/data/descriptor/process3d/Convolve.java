...
public class Convolve {
    ...
    public static ImagePlus convolve(ImagePlus image, float[][][] kernel) {
        ...
        for (int i = 0; i < d; i++) {
            slices_in[i] = image.getStack().getProcessor(i + 1).getPixels();
        ...
        for (int z = 0; z < d; z++) {
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    slices_out[z][y * w + x] = convolvePoint(z, y, x);
                }
            }
        }
        ...
        for (int z = 0; z < d; z++) {
            stack.addSlice("", new FloatProcessor(w, h, slices_out[z], null));
        }
        ...
        for (int k = -r_z / 2; k <= +r_z / 2; k++) {
            for (int j = -r_y / 2; j <= +r_y / 2; j++) {
                for (int i = -r_x / 2; i <= +r_x / 2; i++) {
                    sum += getValue(x + i, y + j, z + k) * H[k + r_z / 2][j + r_y / 2][i + r_x / 2];
                }
            }
        }
        return sum;
